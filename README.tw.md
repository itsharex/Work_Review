...
| 桌面化身 Beta | 用獨立桌寵窗口反饋待機 / 辦公 / 閱讀 / 會議 / 音樂 / 視頻 / 生成中等狀態 |
...
- 支持桌寵大小與貓體透明度調節
- 當前仍處於 `Beta` 階段，會繼續優化切換速度、表情和視覺細節
...
> Linux 目前支持**窗口級追踪**（通過 xdotool/xprop 獲取窗口標題與應用名稱），尚未支持瀏覽器 URL 級追踪。
...
然後到 `系統設置 > 隱私與安全性 > 屏幕錄製` 為 Work Review 開啟權限。

如果你要使用 `桌面化身 Beta`，還需要同時為 Work Review 開啟 `輔助功能` 權限，以便讀取前台窗口狀態並及時切換桌寵表情。
</details>
