...
当前仓库这一轮更新，README 里值得同步的变化主要有这几项：

- **日报页会区分“当前这份日报的实际生成方式”和“当前配置”**
  例如：当前日报可能是“基础模板”，但当前配置仍是 `AI 增强 · gpt-5.4`
- **当 AI 增强未真正生效时，日报页会给出友好中文原因**
  例如：`AI 未生效：请求失败，已回退到基础模板`
- **时间线详情图改为更快打开**
  列表会优先预热首屏重点卡片缩略图，点开详情时会先显示已有缩略图，再并行拉取最新活动详情与高清图
- **时段摘要信息层次更完整**
  除了主摘要外，还会补充节奏标签、副摘要和主应用数量，减少“只有一句话”的单薄感
- **桌面化身 Beta 的状态提醒更细**
  同一工作大类下的细分上下文切换也会触发更贴合当前场景的短气泡反馈
...
| 日报生成 | 生成结构化日报，支持历史回看、按小时活跃度摘要、AI 附加提示词、Markdown 导出、按当前语言切换日报版本，并明确展示当前日报实际生成方式与 AI 回退原因 |
| 双模式回答 | 可选基础模板或 AI 增强，兼顾零配置和表达质量 |
| 桌面化身 Beta | 用独立桌宠窗口反馈待机 / 办公 / 阅读 / 会议 / 音乐 / 视频 / 生成中等状态，并对编码 / 写作 / 规划 / 沟通等细分上下文给出更贴合的提醒 |
...
<img src="docs/Introduction_zh/概览.png" alt="Work Review 日报与时段摘要相关能力" />

日报页现在不仅会显示内容本身，还会明确说明：

- 当前这份日报实际是基础模板还是 AI 增强生成
- 当 AI 没有真正生效时，会给出友好中文回退原因
- 当前配置中的模型与模式是什么，便于区分“配置如此”还是“这份日报实际如此”

时段摘要页也从单句列表升级成更完整的阶段卡片，适合快速扫一遍当天节奏。
...
| **时间线** | 逐时段回看窗口、截图、OCR 文本和页面访问轨迹，并可修正应用分类；详情会优先显示已有缩略图，减少点开后的等待感 |
| **助手** | 用自然语言直接提问，让记录变成可消费的信息 |
| **日报** | 查看、生成和回看任意日期的日报内容，支持按当前界面语言切换日报版本，并支持附加提示词、Markdown 导出、实际生成方式展示和 AI 回退原因提示 |
