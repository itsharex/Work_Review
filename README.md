...
- 支持轻量模式、日报 Markdown 导出和多屏截图策略切换
...
| 日报生成 | 生成结构化日报，支持历史回看、AI 附加提示词和 Markdown 导出 |
...
概览页会把当天的总时长、办公时长、浏览器使用、网站访问和应用分布放在同一屏里，适合先快速判断今天的工作重心。
...
| 页面 | 做什么 |
|------|-------|
| **概览** | 聚合今天的总时长、办公时长、浏览器时长、网站访问和应用分布 |
| **时间线** | 逐时段回看窗口、截图、OCR 文本和页面访问轨迹，并可修正应用分类 |
| **助手** | 用自然语言直接提问，让记录变成可消费的信息 |
| **日报** | 查看、生成和回看任意日期的日报内容，并支持附加提示词和 Markdown 导出 |
