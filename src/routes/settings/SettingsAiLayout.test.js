...
test('刷新模型列表后应给出反馈，且仅在模型为空时才默认回填首项', async () => {
  ...
  assert.doesNotMatch(source, /!fetchedModels\.includes\(config\.text_model\.model\)/);
...
test('下拉列表应只展示实际返回的模型，并明确区分手动输入值', async () => {
  ...
  assert.match(source, /const MANUAL_MODEL_VALUE = '__manual_model__';/);
  assert.match(source, /settingsAI\.manualModelMissing/);
  assert.match(source, /#each fetchedModels as model \(model\)/);
  assert.doesNotMatch(source, /\{#key `\$\{selectedModel\}/);
});
